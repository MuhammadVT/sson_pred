...
class PredSumry(object):
    ...
    """
    def __init__(self, modelPredFname, useSML=True, binTimeRes=60, nBins=1,
    ...
    def create_pred_plots(self, paramDBDir, omnDbName,
        ...
        if binPlotType:
            ...
            dtms = [datetime.datetime(2014, 4, 21, 12, 30),
                    datetime.datetime(2016, 12, 14, 18, 0),
                    datetime.datetime(2014, 3, 30, 15, 0),
                    datetime.datetime(2016, 11, 28, 21, 30)]
            figs = []
            for ind, row in self.predDF.iterrows():
                if row["date"] not in dtms:
                    continue
                _actBinLabs = row[actBinCols].tolist()
                _prBinLabs = row[predBinCols].tolist()
                ...
                print("plotting date--->", _dt)
                fig = esObj.generate_onset_plot(_dt, _actBinLabs, _prBinLabs,\
